...
class Eungalog :
	...
	def select_weather_drip (self):
		result = ""
		if self.weather == "1" :
			result = u"아름다운 날씨이다. 눈이 부시다.. 크극.. 결계인가..?"
		elif self.weather == "2" :
			result = u"거사를 치루고 나니 어둠의 다크가 밀려온다.."
		elif self.weather == "3" :
			result = u"날씨가 구리구리 말똥구리."
		elif self.weather == "4" :
			result = u"바지가 젖었다.. 아니.. 다른이유는 아니고 비 때문에.."
		elif self.weather == "5" :
			result = u"다음번에는 눈에다가 똥을 싸보고 싶다. 낭만적이군."
		elif self.weather == "6" :
			result = u"창밖에 불벼락이 내리고 있다. 세상이 멸망하려나?"
		return result

	def select_size_drip (self):
		result = ""
		if self.size == "1" :
			result = u"내 안에 흑룡이 날뛰고 있었다."
		elif self.size == "2" :
			result = u"하여튼 엄청난 것이 또라이를 틀고 있었다.."
		elif self.size == "3" :
			result = u"뭐, 보통의 똥이었다.(새침)"
		elif self.size == "4" :
			result = u"조금 실망스러운 사이즈였다. 이것이 나의 그릇이란 말인가.."
		elif self.size == "5" :
			result = u"물을 내리기가 미안하다."
		elif self.size == "6" :
			result = u"실패한 고통보다 최선을 다하지 못했음을 깨닫는 것이 더 고통스럽다."
		return result

	def select_feature_drip (self):
		result = ""
		if self.feature == "1" :
			result = u"그치만 특별한 것은 없다."
		elif self.feature == "2" :
	...
	def select_satisfaction_drip (self):
		...
		if self.satisfaction == "1" :
			result = u"크크크크......캬컄캬컄하하핳!! 쿠퀙웩..쿨ㄹ룩.. (실신)"
